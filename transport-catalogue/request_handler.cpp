...
void RequestHandler::MakeRenderMap() {
	std::unordered_set<const domain::Bus*> buses = db_.GetBusToRender();
	...
	std::unordered_set<const domain::Stop*> stops = db_.GetStopToRender();
    
    std::vector<const domain::Stop*> sort_stops(std::move_iterator(stops.begin()), std::move_iterator(stops.end()));
